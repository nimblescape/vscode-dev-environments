...
- **Preview.** For each owner of the repositories that the view has already loaded (no new request to GitHub): how many repositories each entry takes, how many match none, the resulting tree (collapsible, the nodes expanded as in the view), the repositories that would be hidden, and the repositories that match none but stay because they have an environment. The preview uses the same code as the view, so it shows what the view will show. It follows the updates of the view.
- **Test a repository name.** A repository name, or `owner/name`, shows which entry matches it first and the place of its row (owner, name of the entry, levels, and label), or that none matches.
...
- **Changes in settings.json meanwhile.** When the setting changes outside the editor, the editor says so and offers **Load Setting** (which shows the current setting and discards the changes of the editor). Save never overwrites such a change: it reads the current value and applies only the changes made in the editor (additions, removals, edits, and moves) to it, a 3-way merge with the value that the editor loaded as base. Entries that only `settings.json` added or changed stay. Only an entry that both sides changed differently (also removed on one side and edited on the other) is asked about, one question per entry: **Keep Mine** or **Keep settings.json**; dismissing the question saves nothing. After Save, the editor shows the written value.
- **Cancel** or closing the tab discards the changes that were not saved.
