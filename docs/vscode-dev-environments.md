...
- **Changes in settings.json meanwhile.** When the setting changes outside the editor, the editor says so and offers **Load Setting** (which shows the current setting and discards the changes of the editor). Save never overwrites such a change: it reads the current value and applies only the changes made in the editor to it, a 3-way merge with the value that the editor loaded as base. The entries of `settings.json` are matched to the loaded entries over the whole list (equal entries, then the same pattern, then the same name, then the remaining entries after the same neighbor), so an entry keeps its identity when it was moved or edited in `settings.json`. Per entry, the change of the side that changed it wins: additions, removals, and edits of the editor are applied; entries that only `settings.json` added or changed stay, also duplicates. An entry that both sides changed in the same way is no question. Only an entry that both sides changed differently (also removed on one side and edited on the other) is asked about, one question per entry, which shows the entry as loaded, in the editor, and in `settings.json`: **Keep Mine** or **Keep settings.json**. The order: of the side that moved entries; when both sides moved entries differently, one question about the order (**Keep Mine** or **Keep settings.json**); the entries that only one side has follow their predecessor on that side. Dismissing a question saves nothing. After Save, the editor shows the written value.
