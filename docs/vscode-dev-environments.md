...
- **Changes in settings.json meanwhile.** When the setting changes outside the editor, the editor says so and offers **Load Setting** (which shows the current setting and discards the changes of the editor). Save never overwrites such a change: it reads the current `settings.json` and applies your changes to it, and only those: the entries you edited, removed, added, or moved. It finds each entry you changed in `settings.json` by its content (pattern, name, and flags), not by guessing. Everything else in `settings.json` stays as it is, also entries added or changed there. Save asks only where `settings.json` changed or removed the same entry that you edited, or holds a different number of copies of an entry that you changed so that the copy cannot be told, and once when both sides moved entries differently. Such a question shows your entry and the current list of `settings.json`: **Keep Mine** applies your change to that list (your edited entry is added when `settings.json` no longer has the old one), **Keep settings.json** leaves the list as it is for that entry. Nothing else is asked: when `settings.json` made the same change as you, or no longer has an entry that you removed, Save does not ask. An entry you added goes after the entry before it in the editor; an entry that `settings.json` already added is not added twice. When a question is open and `settings.json` changes again, the answers are dropped and Save asks again with the new list. When `settings.json` holds a value that is not a list, Save asks first ("settings.json holds a value for devEnvLauncher.repositoryGroups that is not a list.", with the value): **Replace with Mine** writes the entries of the editor, Cancel saves nothing. Dismissing a question saves nothing. After Save, the editor shows the written value.
