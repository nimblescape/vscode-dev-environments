...
- **Test a repository name.** A repository name, or `owner/name` (at most 140 characters), shows which entry matches it first and the place of its row (owner, name of the entry, levels, and label), or that none matches.
...
- **Changes in settings.json meanwhile.** When the setting changes outside the editor, the editor says so and offers **Load Setting** (which shows the current setting and discards the changes of the editor). Save never overwrites such a change: it reads the current value and applies only the changes made in the editor to it, a 3-way merge with the value that the editor loaded as base. The entries of `settings.json` are matched to the loaded entries first in order (equal entries in the same order), then over the whole list (equal entries, then the same pattern, then the same name, then the remaining entries after the same neighbor), so an entry keeps its identity when it was moved or edited in `settings.json`, and an equal entry added or removed next to it is no move. Per entry, the change of the side that changed it wins: additions, removals, and edits of the editor are applied; entries that only `settings.json` added or changed stay, also duplicates. An entry that both sides changed in the same way is no question. Only an entry that both sides changed differently (also removed on one side and edited on the other) is asked about, one question per entry, which shows the entry as loaded, in the editor, and in `settings.json`: **Keep Mine** or **Keep settings.json**. The order: of the side that moved entries, judged on the entries that both sides kept; when both sides moved entries differently, one question about the order (**Keep Mine** or **Keep settings.json**); the entries that only one side has follow their predecessor on that side. Each question reads `settings.json` again: when an entry that was already asked about changed there meanwhile, it is asked about again with its new value. When `settings.json` holds a value that is not a list, Save asks first ("settings.json holds a value for devEnvLauncher.repositoryGroups that is not a list.", with the value): **Replace with Mine** writes the entries of the editor, Cancel saves nothing. Dismissing a question saves nothing. After Save, the editor shows the written value.
