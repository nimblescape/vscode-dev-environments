...
- **Accounts.** An environment belongs to the GitHub account that created it (see [7.5](#75-environment-model-and-workspace-volume)). Another account never sees or uses it, and the repository list of one account is never shown to another. Each account has its own environment of a repository ([D-3](#13-decisions)), so nothing tells the signed-in account that another account has an environment of a repository. Limit: all environments are Docker volumes of the same user of the computer. The separation protects against the use of the wrong account, not against another person who can use this user of the computer. For that, use separate users of the computer.
...
| D-3 | Number of environments per repository | One environment per repository and GitHub account. It uses the default branch first and switches to other branches on demand. | Several environments per repository and account are not required for now. Each account has its own environment, so one account never learns about or blocks the environment of another (see [7.5](#75-environment-model-and-workspace-volume)). Cost: each account has its own clone on the disk. |
