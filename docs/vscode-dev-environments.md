...
2. The extension checks that Docker is installed. User decision 2026-09-26: "when no remote docker is configured and local docker is not available, the repositories shall not be shown, instead, the side view shall show the install docker wizard". Local Docker is not available when the Docker command line tool is not found; Docker that is installed but not running is available, because the extension starts it when needed (FR-14). Remote Docker hosts do not exist yet (unit 7). While the setup is required, the view shows no repositories, no owners, no hints, and no rows at all, but the Docker setup as its welcome view, with the steps of the walkthrough, each with its button, numbered per platform: "Dev Environments runs your environments in Docker, which is not installed on this computer. Set it up in these steps:"; on Windows, "1." WSL 2 with **Install WSL 2** (once WSL 2 is ready: "✓ WSL 2 is installed." instead of the button); Install Docker Desktop (macOS, Windows) or Docker Engine (Linux) with **Install Docker**; "After the installation, your repositories appear here. Dev Environments starts Docker when it is needed." with **Open the Setup Guide** (the full walkthrough); then the sign-in while not signed in. The entries of the list (loading, empty list, list not loaded) do not show under the setup. The list is still loaded in the background, so it appears at once when Docker is found; commands on repositories from the Command Palette (for example Switch environment) keep working and say "Docker Desktop is not installed." with **Install Docker…**. The first time the view shows, the message "Docker Desktop is not installed." with the action **Install Docker…** appears as well. While Docker is missing, the extension looks for the Docker command line tool every 10 seconds, so the view shows the repositories as soon as Docker is installed. **Install Docker…** opens the walkthrough "Set up Docker for Dev Environments", a wizard with one step per task, each checking itself off: on Windows, WSL 2 (`wsl --install`); Install Docker; Start Docker (Docker Desktop shows its own dialogs once at its first start); Sign in with GitHub. **Install Docker** runs the installer of the computer's platform (see [implementation notes 6](implementation-notes.md#6-docker)): on macOS with Homebrew and on Windows with winget in a terminal of VS Code, otherwise it downloads the installer of Docker Desktop from Docker and opens it; on Linux it installs Docker Engine from the package repository of Docker in the terminal. Before anything runs, a modal dialog lists the exact commands (or the download address and the target file) and says that an administrator password may be requested in the terminal. After the installation, the extension looks for Docker every 5 seconds for at most 30 minutes and then offers **Start Docker**. The installation runs only from a local window; a remote window says "Open a local window to install Docker." If Docker is installed but not running, the extension starts it later, when an environment needs it. Git on the computer is not needed, because the workspace helper contains Git.
...
- **Nodes.** A repository goes under the first regular expression that matches it. Nodes with the same labels under the same parent are one node, also when different regular expressions make them. An entry with a `name` gets its own node with that name in each owner where a repository matches it; it holds the levels of that entry and is never merged with the nodes of other entries.
- **Order.** At every level, the nodes and rows are in the order of the regular expressions in the setting (user decision 2026-09-26): what the first entry puts there comes first. A node that several entries make counts from the first of them. Within one entry, the group nodes come first in alphabetical order, then the rows in alphabetical order of their label. Rows with an environment that match no entry come last.
...
**Repository groups editor.** The Settings editor of VS Code cannot edit this list (it only offers "Edit in settings.json"). **Dev Environments: Edit Repository Groups…** (Command Palette, the **…** menu of the view title bar, and a link in the description of the setting) opens an editor in a tab:

- **Entries.** The entries in their order, each with an optional name, the regular expression, and the flags `i`, `u`, and `s` as check boxes; **Add Entry**, **Up**, **Down**, and **Remove**. Each entry shows its error at once, with the same checks as the sidebar (an empty or invalid regular expression; the error text of JavaScript), and a note about its capturing groups. Capturing groups are not required: an entry without one only filters. An entry of the wrong type in `settings.json`, and flags other than `i`, `u`, and `s`, are named when the editor opens; Save removes them.
- **Preview.** For each owner of the repositories that the view has already loaded (no new request to GitHub): how many repositories each entry takes, how many match none, the resulting tree (collapsible, the nodes expanded as in the view), the repositories that would be hidden, and the repositories that match none but stay because they have an environment. The preview uses the same code as the view, so it shows what the view will show. It follows the updates of the view. The preview and the test run the regular expressions in a separate thread with a limit of 1 second: a regular expression that takes longer for the names of the view (for example a nested repetition such as `(\w+)+$`) stops the preview instead of VS Code, is marked as too slow, and cannot be saved. When the thread fails, for example because a regular expression overflows the stack, the editor says "The preview could not check these regular expressions; Save is not possible.", and Save stays off. The note about the capturing groups counts them in the text of the regular expression, without running it: no regular expression of the editor runs in the extension host. The view itself runs the saved regular expressions in the extension host: a slow one would slow down the view and VS Code (see "Slow patterns" above); a regular expression that fails while it is matched there (for example with a stack overflow) does not match that name, and the log names it once.
- **Test a repository name.** A repository name, or `owner/name` (at most 140 characters), shows which entry matches it first and the place of its row (owner, name of the entry, levels, and label), or that none matches. The text stays after Save and after Load settings.json.
- **Limits.** The editor takes at most 200 entries, names of at most 200 characters, and regular expressions of at most 5000 characters. An entry over a limit (for example from `settings.json`) shows the error at that entry, the status says that the preview shows the entries before, and Save stays off until it is corrected; the edits stay in the editor.
- **Save** writes the setting to the user settings (the setting has the scope `application`). An entry without name and flags is written as a string, any other as an object with only the properties it has; an empty list removes the setting. Only the key `devEnvLauncher.repositoryGroups` changes: the other settings and the comments of `settings.json` stay. Save is possible only when no entry has an error; the extension checks the entries again before it writes.
- **Changes in settings.json meanwhile.** Save writes only this one setting (user decision A, 2026-09-26): it replaces the value of `devEnvLauncher.repositoryGroups` in the user `settings.json` with the entries of the editor, and every other setting and comment stays. Entries are never merged one by one. When the setting changes in `settings.json` while the editor is open, the editor shows the banner "settings.json changed this setting." with **Load settings.json** (shows the current value and drops the unsaved edits); the entries in the editor stay until the user chooses, also when the editor has no unsaved edits: the editor never replaces them on its own. When the value in `settings.json` is no longer the one the editor loaded (or last saved), Save asks first ("devEnvLauncher.repositoryGroups was changed in settings.json while this editor was open.", with the current list of `settings.json`, one entry per line, long entries and long lists cut): **Load settings.json** shows that value and drops the unsaved edits, nothing is written; **Save Mine** replaces that one value with the entries of the editor; **Cancel** changes nothing and keeps the edits. When `settings.json` changes again while the question is open, Save asks again with the new value. When `settings.json` holds a value that is not a list, Save asks the same way, and the question says that the value is not a list. After Save, the editor shows the written value.
- **Cancel** or closing the tab discards the changes that were not saved, also during a Save that still waits for the check or for an answer: nothing is written then.
- The editor is a webview with a strict content security policy: no remote content, only the script and the style sheet of the extension (the script with a nonce). The extension checks every message of the webview and never trusts its content.
...
| Switch | Select another environment in the switcher | The same window connects to the other environment. The previous environment stops after the waiting time, unless it keeps running when closed. |
...
| `contributes.commands`, `contributes.keybindings`, `contributes.menus` | Start, Stop, Delete, Switch branch, Select configuration, Rebuild, Switch environment, Refresh, Install Docker…, Edit Repository Groups… | Command Palette, switcher, menus. The buttons of the walkthrough (install, start, WSL 2) are commands too, hidden in the Command Palette. |
| `contributes.viewsWelcome` | The Docker setup while `devEnvironments.dockerSetupRequired` (intro; on Windows WSL 2 with **Install WSL 2** or "✓ WSL 2 is installed."; **Install Docker** per platform with `isMac`, `isWindows`, `isLinux`; **Open the Setup Guide**), Sign in with GitHub, then loading, empty list, and list not loaded (these three only while the setup is not required) | Welcome texts of the empty view (see [6.1](#61-first-start)) |
...
| Switch to another environment in the same window | The old environment is no longer referenced. The new environment has a pending connection file. | The old environment stops after the waiting time, unless it is kept. |
...
- The switch cannot take effect on a stop that has already begun. The monitor checks it right before `docker stop`; when the user sets it after that point, the `docker stop` that runs (up to 10 seconds, see **Why `docker stop`** below) is finished, and the container is stopped. The switch stays set, so the next **Start** runs the environment as kept.
- A Session Monitor of an older version of the extension does not know the switch and would stop a kept environment. The monitor therefore has a protocol version (see [implementation notes, section 12](implementation-notes.md#12-session-monitor)). A window that finds a running monitor of a known, older version asks it to exit and starts the current one, which takes over when the older one has ended. The window only asks and never sends a signal: the monitor finishes its current step first, including a `docker stop` that has started. A restart of VS Code alone does not reliably retire a monitor of such a version: it ends only when no VS Code window is alive, no pending connection file is fresh (2 minutes), and no waiting time runs (30 seconds by default, `devEnvLauncher.waitingTimeSeconds`). Before it ends, it stops kept environments one last time, because it does not know the switch. After updating from a version without monitor versions: (1) quit VS Code (all windows); (2) wait at least the waiting time (up to 2 minutes plus the waiting time when a connection was just being opened), or until the Session Monitor log `<global storage>/monitor.log` (the global storage folder of the extension, for example `~/.config/Code/User/globalStorage/nimblescape.vscode-dev-environments/monitor.log` on Linux, `~/Library/Application Support/Code/User/globalStorage/nimblescape.vscode-dev-environments/monitor.log` on macOS, `%APPDATA%\Code\User\globalStorage\nimblescape.vscode-dev-environments\monitor.log` on Windows) shows `Session Monitor ends`; (3) then reopen VS Code, and start kept environments again that the old monitor stopped.
- The sidebar shows the state text with ` · kept` in every state, also while the environment is stopped (`Stopped · kept`, see [6.2](#62-sidebar-view)).
...
2. **Empty window at start.** VS Code does not always restore the window. Example on macOS: the user closes the last window, and VS Code continues to run without a window. Later, VS Code opens a new, empty window. In this case, the extension opens the last used environment itself, if all of these conditions are true:
   ...
   - A reopen record exists, and it was older than 5 seconds when the window began to activate.
...
The age condition has a reason. The VS Code command **Close Remote Connection** also changes a window to an empty window. The extension then activates again within 1 to 3 seconds, and the reopen record is younger than 5 seconds (`REOPEN_MIN_AGE_MS` in `src/vscode/activationRules.ts`; the reason in the log is derived from it). So the extension does not reconnect a window that the user disconnected on purpose. The age is measured at the start of the activation, before anything else runs: the checks before the reopen decision (the window status, the pending operations, the GitHub account, and a pause of 3 seconds for windows that VS Code restores at the same time) take several seconds, and measured at the decision, the record of a **Close Remote Connection** would be older than 5 seconds. The limit was 30 seconds until the user decision of 2026-09-26 ("go with the proposal for closing"): the log showed a real reopen from the macOS Dock that the 30-second rule blocked. For a kept environment (see [7.9](#79-stop-on-close-and-crash-handling)), **Close Remote Connection** stops nothing, and the same reopen rules apply. The reopen rule is decision [D-5](#13-decisions).
...
4. The Session Monitor stops the previous environment after the waiting time (see [7.9](#79-stop-on-close-and-crash-handling)), unless it is kept (**Keep Running When Closed**): a kept environment keeps running until the user stops it.
...
| `devEnvLauncher.repositoryGroups` | `[]` | Regular expressions that group the repositories of the sidebar in levels by their capturing groups, and hide the repositories that match none in each owner where at least one repository matches (see [6.2](#62-sidebar-view)). An entry is the regular expression, or `{ "name", "pattern", "flags" }`. Only the user settings can set it (scope `application`): a workspace cannot bring regular expressions that make the view slow. **Dev Environments: Edit Repository Groups…** (also in the **…** menu of the view title bar and linked from the setting) edits it with checks, a preview, and a test field (see [6.2](#62-sidebar-view)); its Save changes only this key in the user `settings.json`, and asks first when that value was changed there meanwhile. |
