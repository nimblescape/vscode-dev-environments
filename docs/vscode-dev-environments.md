...
| `devEnvLauncher.hostAccessChecksOff` | `[]` | Repositories (`owner/name`, compared without case and surrounding spaces) whose host access checks are off (see [section 9](#9-security-and-privacy), "Host access", switch). Scope `application`: only the user settings count, so neither a workspace, a folder, nor a repository can turn a check off. Invalid entries are ignored, with one warning. **Turn Off Host Access Checks…** (after a modal warning) and **Turn On Host Access Checks** in the context menu of a repository row change it. |
