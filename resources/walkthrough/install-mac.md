...
- **With Homebrew:** this command runs in a terminal of VS Code, where you can follow it:
  ...
  If Homebrew still lists Docker Desktop but the app is missing (neither in /Applications nor in ~/Applications), Homebrew first removes its old entry with `brew uninstall --cask --force docker-desktop` (your Docker data stays), then installs Docker Desktop.
