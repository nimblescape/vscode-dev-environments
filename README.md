...
- **Save** writes the user settings and changes only this setting in `settings.json`; your other settings and comments stay. If you changed the setting in `settings.json` while the editor was open, Save applies your changes to the current `settings.json`: the entries you edited, removed, added, or moved; everything else there stays. It asks only where `settings.json` changed or removed an entry that you changed too, or where both moved entries differently; nothing else is asked. If `settings.json` holds a value that is not a list, Save asks before it replaces it. **Cancel**, or closing the tab, discards your changes; a Save that is still waiting then writes nothing.
