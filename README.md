...
| `devEnvLauncher.hostAccessChecksOff` | `[]` | Repositories (`owner/name`) whose host access checks are off (see below). Only the user settings count: a workspace or folder setting cannot turn a check off. **Turn Off Host Access Checks…** and **Turn On Host Access Checks** in the context menu of a repository change it. |
